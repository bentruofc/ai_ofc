...
def _extract_audio(path: str):
    ...
    try:
        ...
        print(f"[Aiorbust Batch] 🔊 audio: {wav.shape[1]} channel(s) @ {sr} Hz")
        return {"waveform": wav, "sample_rate": sr}
    except Exception as e:
        print(f"[Aiorbust Batch] audio not extracted: {e}")
        return None
...
class AiorbustImageBatchLoader:
    ...
    def load_next(self, batch_data: str = "{}", video_max_side: int = 1024,
                  ...
                  force_fps: float = 0.0, unique_id=None):
        # ── Parse data ────────────────────────────────────────────────────────
        try:
...
NODE_DISPLAY_NAME_MAPPINGS = {
    # La cle reste AiorbustImageBatchLoader : elle est inscrite dans chaque
    # workflow sauvegarde, seul le libelle affiche change.
    "AiorbustImageBatchLoader": "Aiorbust Image and Video Batch Loader",
}
