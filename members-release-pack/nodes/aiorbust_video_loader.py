"""
AiorBust Video Loader — Charge un fichier vidéo depuis input/ de ComfyUI.
...
class AiorBustVideoLoader:
    ...
    def load_video(self, video):
        if not _fp:
            raise RuntimeError("[AiorBust Video Loader] folder_paths non disponible.")
        video_path = _fp.get_annotated_filepath(video)
        if not os.path.isfile(video_path):
            raise FileNotFoundError(f"[AiorBust Video Loader] Fichier introuvable : {video_path}")
        print(f"🎞️  [AiorBust Video Loader] {os.path.basename(video_path)}")
        return (video_path,)
