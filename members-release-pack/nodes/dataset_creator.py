...
# ─────────────────────────────────────────────────────────────────────────────
PRESET_NAMES   = ["Flat", "Skinny", "Normal", "Curvy", "Large", "Athletic"]
_RETRY_COUNT   = 5
...
def _find_resources_dir():
    ...
    # Fallback: scan ComfyUI custom_nodes for any aiorbust/ofm folder with Resources/
    try:
        import folder_paths
        custom_nodes_dir = os.path.join(folder_paths.base_path, "custom_nodes")
        for folder_name in os.listdir(custom_nodes_dir):
            name_lower = folder_name.lower()
            if "aiorbust" in name_lower or "ofm" in name_lower:
...
class DatasetCreatorNode:
    ...
    def run(
        ...
        try:
            from .nano_banana_aio import NanoBananaAIO, _load_vertex_json_folder
        except ImportError as e:
            return (torch.zeros(1, 64, 64, 3), f"❌ Could not import NanoBananaAIO: {e}")
        ...
        print(f"\n{'═'*60}")
        print(f"🗂️   AIORBUST DATASET CREATOR")
        ...
        for idx, img_path in enumerate(image_files, 1):
            ...
            _m      = _re.search(r'\d+$', key)
            num_str = _m.group(0) if _m else key
        ...
        def _commit_result(idx, img_path, key, result_tensor, used_model, status, error, face_count=0):
            ...
            # Save with swap-derived numbering: swap_01 → viola_01, swap_45 → viola_45
            import re as _re
            _m = _re.search(r'\d+$', key)
            num_str   = _m.group(0) if _m else key
...
# ─────────────────────────────────────────────────────────────────────────────
NODE_CLASS_MAPPINGS = {
    "DatasetCreatorNode": DatasetCreatorNode,
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "DatasetCreatorNode": "Aiorbust Dataset Creator",
}
