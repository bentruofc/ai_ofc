// ---
// Filename: ../ComfyUI_AIOFC/js/advanced_image_loader.js
// ---
...
app.registerExtension({
	name: "Comfy.AIOFC.AdvancedImageLoader",

	async beforeRegisterNodeDef(nodeType, nodeData, app) {
		if (nodeData.name === "AIOFC_AdvancedImageLoader") {
			...
			nodeType.prototype.onNodeCreated = function () {
				...
				container.className = "aiofc-adv-loader-container";
				...
				const getAspectRatioOutput = (aspectRatioNode, slotIndex) => {
					...
					const ratios = aspectRatioNode.type === "AIOFC_WANAspectRatio"
						? WAN_RATIOS
						: SDXL_RATIOS;
								...
								return order.map((imgId) => {
									...
									return `<div class="aiofc-adv-loader-item ${isActive ? "aiofc-adv-loader-item-active" : ""} ${isPast ? "aiofc-adv-loader-item-done" : ""} ${isProcessing ? "aiofc-adv-loader-item-processing" : ""} ${selectionMode ? "aiofc-adv-loader-item-selection" : ""} ${selectedImages.has(String(img.id)) ? "aiofc-adv-loader-item-selected" : ""}" data-id="${img.id}" draggable="${!selectionMode}">
                                        ...
                                        <div class="aiofc-adv-loader-controls">
                                            <label>×</label>
                                            <input type="number" class="aiofc-adv-loader-repeat-input" value="${img.repeat_count || 1}" min="1" max="99" data-id="${img.id}" ${selectionMode ? 'disabled' : ''} />
                                            <button class="aiofc-adv-loader-delete-btn" data-id="${img.id}" title="Delete" ${selectionMode ? 'disabled' : ''}>×</button>
                                        </div>
                                        <div class="aiofc-adv-loader-info">
                                            <div class="aiofc-adv-loader-filename" title="${img.original_name}">${img.original_name}</div>
                                            <div class="aiofc-adv-loader-dimensions">${img.width}×${img.height} (original)</div>
                                        </div>
                                    </div>`;
								}).join("");
							})()}
				...
				const renderTxt2ImgGallery = () => {
					...
					const existingInput = container.querySelector(".aiofc-adv-loader-batch-count-input");
                        ...
                        <div class="aiofc-adv-loader-gallery">
                            ${order.length === 0 ? `<div class="aiofc-adv-loader-empty"><p>No latents added</p><p class="aiofc-adv-loader-hint">Click "Add Empty Latent" to get started (txt2img mode)</p></div>` : (() => {
								...
								return order.map((latentId) => {
									...
									console.log(`[AIOFC AIL ${node.id}] Rendering latent:`, latent);
									...
									return `<div class="aiofc-adv-loader-item ${isActive ? "aiofc-adv-loader-item-active" : ""} ${isPast ? "aiofc-adv-loader-item-done" : ""} ${isProcessing ? "aiofc-adv-loader-item-processing" : ""}" data-id="${latent.id}" draggable="true">
                                        ...
                                        <div class="aiofc-adv-loader-controls">
                                            <label>×</label>
                                            <input type="number" class="aiofc-adv-loader-repeat-input" value="${latent.repeat_count || 1}" min="1" max="99" data-id="${latent.id}" />
                                            <button class="aiofc-adv-loader-delete-btn" data-id="${latent.id}" title="Delete">×</button>
                                        </div>
                                        <div class="aiofc-adv-loader-info">
                                            <div class="aiofc-adv-loader-filename" title="${latent.id}">Latent ${latent.id.substring(0, 8)}</div>
                                            <div class="aiofc-adv-loader-dimensions">${width}×${height}</div>
                                        </div>
                                    </div>`;
								}).join("");
							})()}
				...
				const updateLatentRepeatCount = (latentId, newCount) => {
					...
					if (latent) {
						...
						node.properties.batch_data = JSON.stringify(batchData);
						syncBatchDataWidget();
						const statsEl = container.querySelector(".aiofc-adv-loader-total");
				...
				const setupTxt2ImgEventHandlers = () => {
					...
					const modeDropdown = container.querySelector(".aiofc-adv-loader-mode-dropdown");
					if (modeDropdown)
					...
					const queueAllBtn = container.querySelector(".aiofc-adv-loader-queue-all-btn");
					if (queueAllBtn) queueAllBtn.onclick = queueAllLatents;
					const addLatentBtn = container.querySelector(".aiofc-adv-loader-add-latent-btn");
					if (addLatentBtn) addLatentBtn.onclick = addEmptyLatent;
					const batchAddBtn = container.querySelector(".aiofc-adv-loader-batch-add-btn");
					if (batchAddBtn) {
						batchAddBtn.onclick = () => {
							const countInput = container.querySelector(".aiofc-adv-loader-batch-count-input");
					...
					const deleteAllBtn = container.querySelector(".aiofc-adv-loader-delete-all-btn");
					if (deleteAllBtn) deleteAllBtn.onclick = deleteAllLatents;
					container.querySelectorAll(".aiofc-adv-loader-delete-btn").forEach((btn) => (btn.onclick = (e) => { e.stopPropagation(); deleteLatent(btn.dataset.id); }));
					container.querySelectorAll(".aiofc-adv-loader-repeat-input").forEach((input) => {
				...
				const duplicateLastItem = () => {
					...
					// Save and re-render (renderGallery already dispatches AIOFC_AIL_UPDATED)
					node.properties.batch_data = JSON.stringify(batchData);
					syncBatchDataWidget();
					renderGallery();
				};
							...
							if (output.links && output.links.length > 0) {
								for (const linkId of output.links) {
									...
									if (link) {
										...
										if (targetNode && targetNode.type === "AIOFC_RealityPromptGenerator") {
								...
								for (const output of node.outputs) {
									if (output.links && output.links.length > 0) {
										for (const linkId of output.links) {
											...
											if (link) {
												const targetNode = app.graph.getNodeById(link.target_id);
												if (targetNode && (targetNode.type === "AIOFC_RealityPromptGenerator" || targetNode.type === "AIOFC_BatchImageGenerator")) {
													...
													for (const inputName of imageInputs) {
														...
														if (input?.link) {
															...
															if (ailLink) {
																...
																if (ailNode && ailNode.type === "AIOFC_AdvancedImageLoader" && ailNode.id !== node.id) {
																	try {
				...
				const setupEventHandlers = () => {
					...
					const duplicateBtn = container.querySelector(".aiofc-adv-loader-duplicate-btn");
					if (duplicateBtn) duplicateBtn.onclick = duplicateLastItem;
					const syncBtn = container.querySelector(".aiofc-adv-loader-sync-btn");
					if (syncBtn) syncBtn.onclick = smartSync;
					const deleteAllBtn = container.querySelector(".aiofc-adv-loader-delete-all-btn");
					if (deleteAllBtn) deleteAllBtn.onclick = deleteAllImages;
					container.querySelectorAll(".aiofc-adv-loader-delete-btn").forEach((btn) => (btn.onclick = (e) => { e.stopPropagation(); deleteImage(btn.dataset.id); }));
					container.querySelectorAll(".aiofc-adv-loader-repeat-input").forEach((input) => {
					...
					const enterSelectionBtn = container.querySelector(".aiofc-adv-loader-enter-selection-btn");
					...
					const cancelSelectionBtn = container.querySelector(".aiofc-adv-loader-cancel-selection-btn");
					...
					const selectAllBtn = container.querySelector(".aiofc-adv-loader-select-all-btn");
					if (selectAllBtn) {
					...
					const deselectAllBtn = container.querySelector(".aiofc-adv-loader-deselect-all-btn");
					if (deselectAllBtn) {
					...
					const deleteSelectedBtn = container.querySelector(".aiofc-adv-loader-delete-selected-btn");
					...
					// Checkbox handlers
					container.querySelectorAll(".aiofc-adv-loader-image-checkbox").forEach((checkbox) => {
						checkbox.onchange = (e) => {
							e.stopPropagation();
					...
					if (selectionMode) {
						container.querySelectorAll(".aiofc-adv-loader-item").forEach((item) => {
							item.onclick = (e) => {
				...
				const setupDragAndDrop = () => {
					const items = container.querySelectorAll(".aiofc-adv-loader-item");
					...
					items.forEach((item) => {
						item.addEventListener("dragstart", (e) => {
							...
							e.stopPropagation();
							e.dataTransfer.setData("text/plain", "aiofc-reorder");
						});
						item.addEventListener("dragend", () => {
							item.style.opacity = "1";
							items.forEach((i) => i.classList.remove("aiofc-adv-loader-drop-before", "aiofc-adv-loader-drop-after"));
						});
						item.addEventListener("dragover", (e) => {
							...
							items.forEach((i) => i.classList.remove("aiofc-adv-loader-drop-before", "aiofc-adv-loader-drop-after"));
							item.classList.add(e.clientY < midpoint ? "aiofc-adv-loader-drop-before" : "aiofc-adv-loader-drop-after");
						});
				...
				const setupFileDropZone = () => {
					...
					const clearDragHighlight = () => {
						dragCounter = 0;
						container.classList.remove("aiofc-adv-loader-drag-over");
						if (dragTimeout) {
					...
					container.addEventListener("dragenter", (e) => {
						...
						if (e.dataTransfer.types.includes("Files")) {
							container.classList.add("aiofc-adv-loader-drag-over");
					...
					container.addEventListener("drop", (e) => {
						...
						if (e.dataTransfer.getData("text/plain") === "aiofc-reorder") {
				...
				const setupWidgetCallbacks = () => {
					const modeWidget = node.widgets?.find((w) => w.name === "mode");
					if (modeWidget && !modeWidget._aiofc_callback_added) {
						const originalCallback = modeWidget.callback;
						modeWidget.callback = function() {
							if (originalCallback) originalCallback.apply(this, arguments);
							renderGallery();
						};
						modeWidget._aiofc_callback_added = true;
					}

					const batchIndexWidget = node.widgets?.find((w) => w.name === "batch_index");
					if (batchIndexWidget && !batchIndexWidget._aiofc_callback_added) {
						const originalCallback = batchIndexWidget.callback;
						batchIndexWidget.callback = function() {
							if (originalCallback) originalCallback.apply(this, arguments);
							renderGallery();
						};
						batchIndexWidget._aiofc_callback_added = true;
					}
					...
					if (aspectLabelWidget && !aspectLabelWidget._aiofc_callback_added) {
						...
						aspectLabelWidget._aiofc_callback_added = true;
					}
				};
				...
				api.addEventListener("aiofc_adv_loader_update", handleBatchUpdate);

				// Listen for Sync requests from RPG
				window.addEventListener("AIOFC_SYNC_AIL_LATENTS", (event) => {
				...
				window.addEventListener("AIOFC_SYNC_AIL_REPEATS", (event) => {
				...
				window.addEventListener("AIOFC_DUPLICATE_LAST_N", (event) => {
				...
				window.addEventListener("AIOFC_TRIM_AIL_IMAGES", (event) => {
				...
				window.addEventListener("AIOFC_SYNC_FROM_MASTER_AIL", (event) => {
