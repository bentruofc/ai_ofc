...
class AIOFC_Interactive_Crop(PreviewImage):
    ...
    def crop_interactively(self, **kwargs):
        ...
        if image is None or timeout is None or uid is None or node_identifier is None:
            raise ValueError("AIOFC_Interactive_Crop is missing required inputs. Check connections.")
        ...
        if bypass:
            if proposed_crop_json and proposed_crop_json.strip():
                try:
                    ...
                    print(f"⏭️ AIOFC Crop BYPASS: Applying proposed crop directly: {crop_data}")
                    return self._apply_crop(image, mask, crop_data)
                except json.JSONDecodeError:
                    print("⚠️ AIOFC Crop BYPASS: Invalid proposed_crop_json, returning full image.")
            else:
                print("⏭️ AIOFC Crop BYPASS: No proposed crop, returning full image.")
        ...
        if cache_behavior == "Resend previous crop" and os.path.exists(cache_filepath):
            print(f"✅ AIOFC Crop Cache Hit! Resending previous crop from {cache_filepath}")
        ...
        if cache_behavior == "Edit previous crop" and os.path.exists(cache_filepath):
            print(f"💨 AIOFC Crop Cache: Loading previous crop for editing from {cache_filepath}")
            with open(cache_filepath, 'r') as f:
            ...
            try:
                ...
                print("⚠️ AIOFC Interactive Crop: Invalid proposed_crop_json. Ignoring.")

        print("💨 AIOFC Interactive Crop: Waiting for user to define crop area...")
        response = send_and_wait(payload, timeout, uid, node_identifier)

        if isinstance(response, TimeoutResponse):
            print("⏰ AIOFC Interactive Crop: Timed out. Passing through original image.")
        ...
        print(f"✅ AIOFC Interactive Crop: Received crop data: {crop_data}")
    ...
    def _apply_crop(self, image, mask, crop_data):
        ...
        if x != crop_data.get("x") or y != crop_data.get("y") or width != crop_data.get("width") or height != crop_data.get("height"):
            print(f"⚠️ AIOFC Crop: Adjusted crop to fit image bounds: x={x}, y={y}, w={width}, h={height}")
