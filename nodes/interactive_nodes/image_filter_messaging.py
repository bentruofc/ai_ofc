...
@PromptServer.instance.routes.post('/aiofc/interactive_message')
...
@PromptServer.instance.routes.post('/aiofc/clear_text_filter_cache')
async def clear_text_filter_cache(request):
    try:
        ...
        uid = data.get('uid')
        print(f"🧹 AIOFC API: Received request to clear text filter cache from node UID {uid}.")

        cache_dir = os.path.join(os.path.dirname(__file__), "..", "..", "cache")
        if not os.path.isdir(cache_dir):
            print("🧹 AIOFC API: Cache directory does not exist. Nothing to clear.")
        ...
        for filename in os.listdir(cache_dir):
            ...
            if filename.endswith("_text_edit.json"):
                ...
                try:
                    ...
                    print(f"⚠️ AIOFC API: Error clearing cache file {file_path}: {e}")
                    # We continue even if one file fails to delete
        
        print(f"✅ AIOFC API: Successfully cleared {cleared_count} text filter cache file(s).")
        return web.json_response({"status": "success", "cleared_count": cleared_count, "files": files_cleared})

    except Exception as e:
        print(f"❌ AIOFC API: An unexpected error occurred while clearing cache: {e}")
...
def wait_for_response(secs, uid, unique) -> Response:
    ...
    try:
        ...
        while(time.monotonic() < end_time and MessageState.waiting()):
            ...
            PromptServer.instance.send_sync("aiofc-interactive-images", {"tick": int(end_time - time.monotonic()), "uid": uid, "unique":unique})
            time.sleep(0.5)
        if MessageState.waiting():
            PromptServer.instance.send_sync("aiofc-interactive-images", {"timeout": True, "uid": uid, "unique":unique})
...
def send_and_wait(payload, timeout, uid, unique) -> Response:
    ...
    while True:
        PromptServer.instance.send_sync("aiofc-interactive-images", payload)
