"""
ComfyUI node — Aiofc Directory Image Loader.
...
try:
    ...
    async def _get_status(request):
        ...
        return web.json_response(DirectoryImageLoaderNode._counters)
    ...
    async def _reset_batch(request):
        ...
        DirectoryImageLoaderNode._batch_runs.clear()
        return web.json_response({"ok": True})
except Exception:
    pass


class DirectoryImageLoaderNode:
    ...
    def load_image(self, use_absolute_path, run_count, input_folder=None, absolute_path=""):
        ...
        if key not in DirectoryImageLoaderNode._counters:
            DirectoryImageLoaderNode._counters[key] = 0
        current = DirectoryImageLoaderNode._counters[key] % total
        DirectoryImageLoaderNode._counters[key] = current + 1

        # Track how many runs done in this batch
        if key not in DirectoryImageLoaderNode._batch_runs:
            DirectoryImageLoaderNode._batch_runs[key] = 0
        DirectoryImageLoaderNode._batch_runs[key] += 1
        batch_done = DirectoryImageLoaderNode._batch_runs[key]
...
NODE_CLASS_MAPPINGS = {
    "DirectoryImageLoaderNode": DirectoryImageLoaderNode,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "DirectoryImageLoaderNode": "Aiofc Directory Image Loader",
}
