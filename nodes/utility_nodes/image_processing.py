...
def _build_phone_exif(profile, width, height, gps_coords=None):
    ...
    exif_ifd[0x829A] = (1, exp_denom)
    exif_ifd[0x829D] = profile["f_number"]
    exif_ifd[0x8822] = 2
    exif_ifd[0x8827] = iso
    exif_ifd[0x9000] = b"0232"
    exif_ifd[0x9003] = dt_str
    exif_ifd[0x9004] = dt_str
    exif_ifd[0x9201] = ssv
    exif_ifd[0x9202] = profile["f_number"]
    exif_ifd[0x9203] = (0, 1)
    exif_ifd[0x9204] = (0, 1)
    exif_ifd[0x9207] = 5
    exif_ifd[0x9209] = 0x10
    exif_ifd[0x920A] = profile["focal_length"]
    exif_ifd[0xA001] = 1
    exif_ifd[0xA002] = width
    exif_ifd[0xA003] = height
    exif_ifd[0xA405] = 26
    exif_ifd[0xA406] = 0
    exif_ifd[0xA433] = profile["make"]
    exif_ifd[0xA434] = profile["lens_model"]
    ...
    if gps_coords:
        ...
        gps_ifd[0] = b"\x02\x03\x00\x00"
        gps_ifd[1] = "N" if lat >= 0 else "S"
        gps_ifd[2] = _decimal_to_dms(lat)
        gps_ifd[3] = "E" if lon >= 0 else "W"
        gps_ifd[4] = _decimal_to_dms(lon)
        gps_ifd[5] = 0
        gps_ifd[6] = IFDRational(int(alt * 100), 100)
        gps_ifd[7] = (IFDRational(now.hour, 1), IFDRational(now.minute, 1), IFDRational(now.second, 1))
        gps_ifd[29] = now.strftime("%Y:%m:%d")
...
NODE_CLASS_MAPPINGS = {
    "MetadataRemoveNode": MetadataRemoveNode,
    "MetadataRemoveNodeMonthly": MetadataRemoveNode,
    "SaveAsPhonePhotoNode": SaveAsPhonePhotoNode,
    "SaveAsPhonePhotoNodeMonthly": SaveAsPhonePhotoNode,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "MetadataRemoveNode": "AIOFC Remove Image Metadata",
    "MetadataRemoveNodeMonthly": "AIOFC Remove Image Metadata (Legacy)",
    "SaveAsPhonePhotoNode": "AIOFC Save as Phone Photo",
    "SaveAsPhonePhotoNodeMonthly": "AIOFC Save as Phone Photo (Legacy)",
}
