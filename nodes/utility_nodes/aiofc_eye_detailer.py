"""
AIOFC HD Detailer nodes
=======================

Patched copies of two nodes from Impact Pack / Impact Subpack:
1. AiofcEyeBBoxDetectorProvider ("AIOFC HD Ultralytic BBox Loader")
2. AiofcDetailer ("AIOFC Detailer")
"""
...
def _get_subcore():
    """Return live `subcore` module from ComfyUI-Impact-Subpack."""
    ...
    raise RuntimeError(
        "[AIOFC] Impossible to find 'subcore' from ComfyUI-Impact-Subpack. "
        "Please ensure Impact Subpack is installed and enabled."
    )
...
class AiofcEyeBBoxDetectorProvider:
    @classmethod
    def INPUT_TYPES(cls):
        import folder_paths
        ...
        return {
            "required": {
                ...
                "imgsz": ("INT", {
                    ...
                    "tooltip": "YOLO inference resolution. Higher = better small object detection (e.g. eyes) but slower."
                }),
            }
        }
    ...
    FUNCTION = "doit"
    CATEGORY = "AIOFC/Detailer"
    DESCRIPTION = "Ultralytics Detector Provider with customizable YOLO inference resolution (imgsz)."
    ...
    def doit(self, model_name, imgsz=1280):
        ...
        if model_path is None:
            raise ValueError(f"[AIOFC] model file '{model_name}' not found.")

        model = subcore.load_yolo(model_path)

        if model_name.startswith("bbox"):
            return AiofcHDBBoxDetector(model, imgsz), subcore.NO_SEGM_DETECTOR()
        else:
            ...
            return AiofcHDBBoxDetector(model, imgsz), segm_detector
...
def enhance_detail_hd(image, model, clip, vae, guide_size, guide_size_for_bbox, max_size, bbox, seed, steps, cfg,
    ...
    if not force_inpaint and bbox_h >= guide_size and bbox_w >= guide_size:
        logging.info("AIOFC Detailer: segment skip (enough big)")
        return None, None
    ...
    if not force_inpaint:
        if upscale <= 1.0:
            logging.info(f"AIOFC Detailer: segment skip [determined upscale factor={upscale}]")
            return None, None

        if new_w == 0 or new_h == 0:
            logging.info(f"AIOFC Detailer: segment skip [zero size={new_w, new_h}]")
            return None, None
    else:
        if upscale <= 1.0 or new_w == 0 or new_h == 0:
            logging.info("AIOFC Detailer: force inpaint")
            upscale = 1.0
            new_w = w
            new_h = h
    ...
    logging.info(f"AIOFC Detailer: segment upscale for ({bbox_w, bbox_h}) | crop region {w, h} x {upscale} -> {new_w, new_h}")
    ...
    if detailer_hook is None or not detailer_hook.get_skip_sampling():
        if noise_mask is not None and inpaint_model:
            ...
            if 'noise_mask' in inspect.signature(imc_encode).parameters:
                ...
                logging.warning("[AIOFC Detailer] ComfyUI is an outdated version.")
        ...
        if vae_tiled_decode:
            ...
            logging.info(f"[AIOFC Detailer] vae decoded (tiled) in {time.time() - start:.1f}s")
        else:
            try:
                refined_image = vae.decode(refined_latent['samples'])
            except Exception:
                logging.warning(f"[AIOFC Detailer] failed after {time.time() - start:.1f}s, doing vae.decode_tiled 64...")
                refined_image = vae.decode_tiled(refined_latent["samples"], tile_x=64, tile_y=64)
            logging.info(f"[AIOFC Detailer] vae decoded in {time.time() - start:.1f}s")
    else:
...
class AiofcDetailer:
    ...
    def INPUT_TYPES(s):
        ...
        return {"required": {
                     "image": ("IMAGE",),
                     "model": ("MODEL", {"tooltip": "If `ImpactDummyInput` is connected, sampling step is skipped."}),
                     "clip": ("CLIP",),
                     "vae": ("VAE",),
                     ...
                     "interpolation": (INTERPOLATION_MODES, {
                         "default": "bicubic",
                         "tooltip": "Resampler used for paste-back after VAE decode."
                     }),
                     "sharpness": ("FLOAT", {
                         "default": 0.5, "min": 0.0, "max": 2.0, "step": 0.05,
                         "tooltip": "Luminance-only unsharp mask applied after paste-back downscale."
                     }),
                     "color_match_strength": ("FLOAT", {
                         "default": 1.0, "min": 0.0, "max": 1.0, "step": 0.05,
                         "tooltip": "Strength of statistical color match against original crop."
                     }),
                     },
                "optional": {
                    "bbox_detector": ("BBOX_DETECTOR", {
                        "tooltip": "Bbox detector. Optional if `segs` is connected."
                    }),
                    "segs": ("SEGS", {
                        "tooltip": "Precalculated SEGS. If connected, bypasses bbox/sam/segm detection."
                    }),
    ...
    FUNCTION = "doit"
    CATEGORY = "AIOFC/Detailer"
    DESCRIPTION = "Face/eye detailer with interpolation, sharpness, and color match controls."
    ...
    def do_detail(image, segs, model, clip, vae, guide_size, guide_size_for_bbox, max_size, seed, steps, cfg, sampler_name, scheduler,
        ...
        if len(image) > 1:
            raise Exception('[AIOFC Detailer] ERROR: image batches are not supported by do_detail.')
        ...
        for i, seg in enumerate(ordered_segs):
            ...
            if is_mask_all_zeros:
                logging.info("AIOFC Detailer: segment skip [empty mask]")
                continue
    ...
    def enhance_face(image, model, clip, vae, guide_size, guide_size_for_bbox, max_size, seed, steps, cfg, sampler_name, scheduler,
        ...
        if segs_opt is not None:
            ...
            if bbox_detector is None:
                raise Exception("[AIOFC Detailer] ERROR: connect either `bbox_detector` or optional `segs`.")
        ...
        if len(segs[1]) > 0:
            enhanced_img, _, cropped_enhanced, cropped_enhanced_alpha, cnet_pil_list, new_segs = \
                AiofcDetailer.do_detail(image, segs, model, clip, vae, guide_size, guide_size_for_bbox, max_size, seed, steps, cfg,
    ...
    def doit(self, image, model, clip, vae, guide_size, guide_size_for, max_size, seed, steps, cfg, sampler_name, scheduler,
        ...
        if not torch.is_tensor(image):
            raise Exception(
                "[AIOFC Detailer] Input `image` is not a valid tensor (received: "
                f"{type(image).__name__})."
            )

        if len(image) > 1:
            logging.warning("[AIOFC Detailer] WARN: this node is not designed for video detailing.")

        for i, single_image in enumerate(image):
            enhanced_img, cropped_enhanced, cropped_enhanced_alpha, mask, cnet_pil_list = AiofcDetailer.enhance_face(
