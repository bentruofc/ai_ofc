"""
AIOFC API Nodes
External API integrations for ComfyUI
"""

from .generative_api_nodes import (
    NODE_CLASS_MAPPINGS as GENERATIVE_API_MAPPINGS,
    NODE_DISPLAY_NAME_MAPPINGS as GENERATIVE_API_DISPLAY_MAPPINGS,
)
from .ideogram import (
    NODE_CLASS_MAPPINGS as IDEOGRAM_MAPPINGS,
    NODE_DISPLAY_NAME_MAPPINGS as IDEOGRAM_DISPLAY_MAPPINGS,
)
from .gemini_native import (
    NODE_CLASS_MAPPINGS as GEMINI_NATIVE_MAPPINGS,
    NODE_DISPLAY_NAME_MAPPINGS as GEMINI_NATIVE_DISPLAY_MAPPINGS,
)
from .grok_native import (
    NODE_CLASS_MAPPINGS as GROK_NATIVE_MAPPINGS,
    NODE_DISPLAY_NAME_MAPPINGS as GROK_NATIVE_DISPLAY_MAPPINGS,
)
from .flux_kontext import (
    NODE_CLASS_MAPPINGS as FLUX_KONTEXT_MAPPINGS,
    NODE_DISPLAY_NAME_MAPPINGS as FLUX_KONTEXT_DISPLAY_MAPPINGS,
)
from .flux_fill import (
    NODE_CLASS_MAPPINGS as FLUX_FILL_MAPPINGS,
    NODE_DISPLAY_NAME_MAPPINGS as FLUX_FILL_DISPLAY_MAPPINGS,
)
from .nano_banana_pro_multi import (
    NODE_CLASS_MAPPINGS as NANO_BANANA_PRO_MULTI_MAPPINGS,
    NODE_DISPLAY_NAME_MAPPINGS as NANO_BANANA_PRO_MULTI_DISPLAY_MAPPINGS,
)
from .parallel_batch_generator import (
    NODE_CLASS_MAPPINGS as PARALLEL_BATCH_MAPPINGS,
    NODE_DISPLAY_NAME_MAPPINGS as PARALLEL_BATCH_DISPLAY_MAPPINGS,
)
from .nano_banana_edit import (
    NODE_CLASS_MAPPINGS as NANO_BANANA_EDIT_MAPPINGS,
    NODE_DISPLAY_NAME_MAPPINGS as NANO_BANANA_EDIT_DISPLAY_MAPPINGS,
)
from .seedream_edit import (
    NODE_CLASS_MAPPINGS as SEEDREAM_EDIT_MAPPINGS,
    NODE_DISPLAY_NAME_MAPPINGS as SEEDREAM_EDIT_DISPLAY_MAPPINGS,
)
from .nano_banana_aio import (
    NODE_CLASS_MAPPINGS as NANO_BANANA_AIO_MAPPINGS,
    NODE_DISPLAY_NAME_MAPPINGS as NANO_BANANA_AIO_DISPLAY_MAPPINGS,
)
from .h3_context_ir import (
    NODE_CLASS_MAPPINGS as H3_CONTEXT_IR_MAPPINGS,
    NODE_DISPLAY_NAME_MAPPINGS as H3_CONTEXT_IR_DISPLAY_MAPPINGS,
)
from .grok_prompt import (
    NODE_CLASS_MAPPINGS as GROK_PROMPT_MAPPINGS,
    NODE_DISPLAY_NAME_MAPPINGS as GROK_PROMPT_DISPLAY_MAPPINGS,
)
from .gemini_prompt import (
    NODE_CLASS_MAPPINGS as GEMINI_PROMPT_MAPPINGS,
    NODE_DISPLAY_NAME_MAPPINGS as GEMINI_PROMPT_DISPLAY_MAPPINGS,
)
from .api_keys_loader import (
    NODE_CLASS_MAPPINGS as API_KEYS_LOADER_MAPPINGS,
    NODE_DISPLAY_NAME_MAPPINGS as API_KEYS_LOADER_DISPLAY_MAPPINGS,
)
from .lora_caption import (
    NODE_CLASS_MAPPINGS as LORA_CAPTION_MAPPINGS,
    NODE_DISPLAY_NAME_MAPPINGS as LORA_CAPTION_DISPLAY_MAPPINGS,
)
from . import creative_api


# Combine all API node mappings
NODE_CLASS_MAPPINGS = {
    **GENERATIVE_API_MAPPINGS,
    **IDEOGRAM_MAPPINGS,
    **GEMINI_NATIVE_MAPPINGS,
    **GROK_NATIVE_MAPPINGS,
    **FLUX_KONTEXT_MAPPINGS,
    **FLUX_FILL_MAPPINGS,
    **NANO_BANANA_PRO_MULTI_MAPPINGS,
    **PARALLEL_BATCH_MAPPINGS,
    **NANO_BANANA_EDIT_MAPPINGS,
    **SEEDREAM_EDIT_MAPPINGS,
    **NANO_BANANA_AIO_MAPPINGS,
    **H3_CONTEXT_IR_MAPPINGS,
    **GROK_PROMPT_MAPPINGS,
    **GEMINI_PROMPT_MAPPINGS,
    **API_KEYS_LOADER_MAPPINGS,
    **LORA_CAPTION_MAPPINGS,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    **GENERATIVE_API_DISPLAY_MAPPINGS,
    **IDEOGRAM_DISPLAY_MAPPINGS,
    **GEMINI_NATIVE_DISPLAY_MAPPINGS,
    **GROK_NATIVE_DISPLAY_MAPPINGS,
    **FLUX_KONTEXT_DISPLAY_MAPPINGS,
    **FLUX_FILL_DISPLAY_MAPPINGS,
    **NANO_BANANA_PRO_MULTI_DISPLAY_MAPPINGS,
    **PARALLEL_BATCH_DISPLAY_MAPPINGS,
    **NANO_BANANA_EDIT_DISPLAY_MAPPINGS,
    **SEEDREAM_EDIT_DISPLAY_MAPPINGS,
    **NANO_BANANA_AIO_DISPLAY_MAPPINGS,
    **H3_CONTEXT_IR_DISPLAY_MAPPINGS,
    **GROK_PROMPT_DISPLAY_MAPPINGS,
    **GEMINI_PROMPT_DISPLAY_MAPPINGS,
    **API_KEYS_LOADER_DISPLAY_MAPPINGS,
    **LORA_CAPTION_DISPLAY_MAPPINGS,
}

__all__ = ["NODE_CLASS_MAPPINGS", "NODE_DISPLAY_NAME_MAPPINGS"]
